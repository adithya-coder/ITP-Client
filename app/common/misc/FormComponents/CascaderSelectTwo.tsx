...
const CascaderSelect = (props: any) => {
    ...
    const { textfieldprops, options, form, label, required, setFieldValue } = props;
    ...
    const generateList: any = (option: any, index: number) => {
        ...
        return option.children && option.children.length > 0 ?
            <div>
                <ListItem style={{ minWidth: 250, backgroundColor: option === selectedOption ? 'lightgray' : '' }} button>
                ...
                <Card style={{position:'fixed', margin:'5px', marginLeft: index % 2 === 0 ? '251px' : '0px', marginTop: index % 2 === 0 ? '-48px' : '0px', zIndex:index+2}}>
            ...
            :
            <ListItem onClick={() => { handleSelect(option) }} style={{ minWidth: 250, backgroundColor: option === selectedOption ? 'lightgray' : 'white' }} button>
                <ListItemText primary={option.label+' ('+option.code+')'} />
            </ListItem>
    }

    return (
        <FormControl size="small" error={!!errorText} className={classes.formSelectors} style={{ width: "100%" }}>
            <InputLabel style={{ marginTop: -7, marginLeft: 14, zIndex: 1 }}>{label + (required ? ' *' : '')}</InputLabel>
            <Select size="small" {...field} {...props} {...textfieldprops} value={defaultValue} style={{ minWidth: 250 }} onClick={handleClick} inputProps={{ readOnly: true }}>
            ...
            >
                {options.length > 0 ?
                    ...
                    :
                    <ListItem onClick={handleClose} style={{ width: 250 }}>
