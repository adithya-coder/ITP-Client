...
const FormikAutocomplete = (props: any) => {
    ...
    const autocompleteDropdown="autocompleteDropdownLight";
   
    const inputFill ="inputLight"

    return (
        <FormControl size='small' fullWidth error={!!errorText} className={`${classes.formSelectors} ${inputFill}`}>
            <Autocomplete
                PaperComponent={({ children }) => (
                    <Paper className={`${autocompleteDropdown} ${autocompleteDropdown}`}>{children}</Paper>
                )}
                className={`${customTheme.autocompleteContent}`}
                // style={{color: '#e4e6eb', backgroundColor: '#3e4042',}}
                size="small"
                options={options}
